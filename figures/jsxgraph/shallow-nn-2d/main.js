...
const p0 = board_output.create('slider', p0param, { ...{ name: '\\(\\beta_0\\)' }, ...satts }),
        p1 = board_output.create('slider', p1param, { ...{ name: '\\(\\beta_1\\)' }, ...satts }),
        p2 = board_output.create('slider', p2param, { ...{ name: '\\(\\beta_2\\)' }, ...satts }),
        p3 = board_output.create('slider', p3param, { ...{ name: '\\(\\beta_3\\)' }, ...satts }),

        t10 = board_h1.create('slider', t0param, { ...{ name: '\\(w_{10}\\)' }, ...satts }),
        t11 = board_h1.create('slider', t1param, { ...{ name: '\\(w_{11}\\)' }, ...satts }),
        t12 = board_h1.create('slider', t2param, { ...{ name: '\\(w_{12}\\)' }, ...satts }),

        t20 = board_h2.create('slider', t0param, { ...{ name: '\\(w_{20}\\)' }, ...satts }),
        t21 = board_h2.create('slider', t1param, { ...{ name: '\\(w_{21}\\)' }, ...satts }),
        t22 = board_h2.create('slider', t2param, { ...{ name: '\\(w_{22}\\)' }, ...satts }),

        t30 = board_h3.create('slider', t0param, { ...{ name: '\\(w_{30}\\)' }, ...satts }),
        t31 = board_h3.create('slider', t1param, { ...{ name: '\\(w_{31}\\)' }, ...satts }),
        t32 = board_h3.create('slider', t2param, { ...{ name: '\\(w_{32}\\)' }, ...satts });
...
// const elSlider = board_output.create('slider', [[sleft, -2], [sright, -2], [0, Math.PI / 4, Math.PI / 2]], { ...{ name: 'El.' }, ...satts }),
//         azSlider = board_output.create('slider', [[sleft, -3], [sright, -3], [0, Math.PI / 4, 2 * Math.PI]], { ...{ name: 'Az.' }, ...satts });

// Text
const textcoords = [-1.5, -3.5];
board_lin_h1.create('text',
    [...textcoords, '\\[w_{10} + w_{11} X_1 + w_{12} X_2\\]'],
    { anchorX: 'left' });

// board_clip_h1.create('text',
//     [...textcoords, '\\[A_1 = g(w_{10} + w_{11} X_1 + w_{12} X_2)\\]'],
//     { anchorX: 'left' });
board_clip_h1.create('text',
    [-1.5, -2.5, '\\[A_1 = \\]'],
    { anchorX: 'left' });
board_clip_h1.create('text',
    [...textcoords, '\\[g(w_{10} + w_{11} X_1 + w_{12} X_2)\\]'],
    { anchorX: 'left' });

board_scale_h1.create('text',
    [...textcoords, '\\[\\beta_1 A_1\\]'],
    { anchorX: 'left' });

board_lin_h2.create('text',
    [...textcoords, '\\[w_{20} + w_{21} X_1 + w_{22} X_2\\]'],
    { anchorX: 'left' });

// board_clip_h2.create('text',
//     [...textcoords, '\\[A_2 = g(w_{20} + w_{21} X_1 + w_{22} X_2)\\]'],
//     { anchorX: 'left' });
board_clip_h2.create('text',
    [-1.5, -2.5, '\\[A_2 =\\]'],
    { anchorX: 'left' });
board_clip_h2.create('text',
    [...textcoords, '\\[g(w_{20} + w_{21} X_1 + w_{22} X_2)\\]'],
    { anchorX: 'left' });

board_scale_h2.create('text',
    [...textcoords, '\\[\\beta_2 A_2\\]'],
    { anchorX: 'left' });

board_lin_h3.create('text',
    [...textcoords, '\\[w_{30} + w_{31} X_1 + w_{32} X_3\\]'],
    { anchorX: 'left' });

// board_clip_h3.create('text',
//     [...textcoords, '\\[A_3 = g(w_{30} + w_{31} X_1 + w_{32} X_3)\\]'],
//     { anchorX: 'left' });
board_clip_h3.create('text',
    [-1.5, -2.5, '\\[A_3 =\\]'],
    { anchorX: 'left' });
board_clip_h3.create('text',
    [...textcoords, '\\[g(w_{30} + w_{31} X_1 + w_{32} X_3)\\]'],
    { anchorX: 'left' });

board_scale_h3.create('text',
    [...textcoords, '\\[\\beta_3 A_3\\]'],
    { anchorX: 'left' });

// board_lin_output.create('text',
//     [...textcoords, '\\[Y = \\beta_0 + \\beta_1 A_1 + \\beta_2 A_2 + \\beta_3 A_3\\]'],
//     { anchorX: 'left' });
board_lin_output.create('text',
    [-1.5, -2.5, '\\[Y =\\]'],
    { anchorX: 'left' });
board_lin_output.create('text',
    [...textcoords, '\\[\\beta_0 + \\beta_1 A_1 + \\beta_2 A_2 + \\beta_3 A_3\\]'],
    { anchorX: 'left' });
